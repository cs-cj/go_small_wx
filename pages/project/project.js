...
Page({
  ...
  onLoad:function(){
    ...
    wx.request({
      url: app.globalData.url + 'projects',
      method: "GET",
      data: {
        p: page//传页码参数，默认展示第一页
      },
      success: function (res) {
        var result = res.data.data.projects;
        // console.info(res.data.data)
        console.info(result)
        var str = "http";
        var img_url;//声明头像路径
        var new_projects_list = [];//新数据
        var hidden_flag;//是否隐藏
        for (var i = 0; i < result.length; i++) {//默认展示前5条数据
          img_url = result[i].logo;//获取头像路径          
          if (img_url.indexOf(str) == -1) {//如果头像路径中没有http字段，则图片无效
            result[i].flag = false;//false表示显示默认图片
          } else {
            result[i].flag = true;
          }
          new_projects_list = that.data.projects_list.concat(result);
        }
        ...
        that.setData({
          projects_list: new_projects_list,
          page: page,
