...
Page({ 
  ...
  send: function (e) {
    ...
    if (user) {
      ...
      wx.request({
        url: app.globalData.url + 'comment/' + that.data.resouce_id,
        data: {
          objtype: parseInt(2),
  ...
  emojiShowHide: function () {
   ...
   getComment:function(){
     var that = this;
     wx.request({
       url: app.globalData.url + 'resource/detail?id=' + that.data.resouce_id,
       // data:{
       //   id:id
       // },
       method: "GET",
       success: function (res) {
         var result = res.data.data.resource;
         console.info(res)
         console.info(result)
         ...
         //回复头像
         var result0 = res.data.data.comments;
         var str = "http";
         var img_url;//声明头像路径
  ...
  onShareAppMessage: function (res) {//设置该页面的转发信息
    return {
      // title: '',
      path: '/pages/resource_detail/resource_detail',
      success: function (res) {
